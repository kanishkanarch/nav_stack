...
#include "ADAstar_ros.h"

#include <pluginlib/class_list_macros.h>
//register this planner as a BaseGlobalPlanner plugin
PLUGINLIB_EXPORT_CLASS(ADAstar_planner::ADAstarPlannerROS, nav_core::BaseGlobalPlanner)
...
namespace ADAstar_planner
{

//Default Constructor
ADAstarPlannerROS::ADAstarPlannerROS()
{

}
ADAstarPlannerROS::ADAstarPlannerROS(ros::NodeHandle &nh)
{
  ROSNodeHandle = nh;

}

ADAstarPlannerROS::ADAstarPlannerROS(std::string name, costmap_2d::Costmap2DROS* costmap_ros)
{
  initialize(name, costmap_ros);
}

void ADAstarPlannerROS::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros)
{
...
bool ADAstarPlannerROS::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal,
...
MyExcelFile << startCell <<"\t"<< start.pose.position.x <<"\t"<< start.pose.position.y <<"\t"<< goalCell <<"\t"<< goal.pose.position.x <<"\t"<< goal.pose.position.y;
  ...
  if (isStartAndGoalCellsValid(startCell, goalCell)){
    ...
    bestPath = ADAstarPlanner(startCell, goalCell);
...
void ADAstarPlannerROS::getCorrdinate(float& x, float& y)
{
...
int ADAstarPlannerROS::convertToCellIndex(float x, float y)
{
...
void ADAstarPlannerROS::convertToCoordinate(int index, float& x, float& y)
{
...
bool ADAstarPlannerROS::isCellInsideMap(float x, float y)
{
  bool valid = true;
...
void ADAstarPlannerROS::mapToWorld(double mx, double my, double& wx, double& wy){
   costmap_2d::Costmap2D* costmap = costmap_ros_->getCostmap();
    wx = costmap->getOriginX() + mx * resolution;
    wy = costmap->getOriginY() + my * resolution;
}

vector<int> ADAstarPlannerROS::ADAstarPlanner(int startCell, int goalCell){
...
/*********************************************************************************/
/*
float ADAstarPlannerROS::calculateHCost(int cellID, int goalCell)
{    
...
vector <int> ADAstarPlannerROS::findFreeNeighborCell (int CellID){
...
/*********************************************************************************/
bool ADAstarPlannerROS::isStartAndGoalCellsValid(int startCell,int goalCell)
{ 
 bool isvalid=true;
 ...
 float  ADAstarPlannerROS::getMoveCost(int i1, int j1, int i2, int j2){
  ...
  float  ADAstarPlannerROS::getMoveCost(int CellID1, int CellID2){
 ...
 bool  ADAstarPlannerROS::isFree(int i, int j){
 ...
 return OGM[CellID];
  ...
  //verify if the cell(i,j) is free
 bool  ADAstarPlannerROS::isFree(int CellID){
 return OGM[CellID];
 } 
}
;
