...
#include "dual_planner.h"

#include <pluginlib/class_list_macros.h>
//register this planner as a BaseGlobalPlanner plugin
PLUGINLIB_EXPORT_CLASS(dual_planner_ns::dual_planner, nav_core::BaseGlobalPlanner)
	...
	namespace dual_planner_ns
{
	...
	void dual_planner::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros)
	{
		...
		if (!initialized_)
		{
			...
			ROS_INFO("Dual line planner initialized successfully");
			initialized_ = true;
		}
		else
	...
	bool dual_planner::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal,
			std::vector<geometry_msgs::PoseStamped>& plan)
	{
		...
		float turnX, turnY, L;
		L = 1;
		if(goal.pose.orientation.z > 0)
		{
			turnX = goal.pose.position.x - L*cos(2*acos(goal.pose.orientation.w));
			turnY = goal.pose.position.y - L*sin(2*acos(goal.pose.orientation.w));
		}
		else
		{
			turnX = goal.pose.position.x - L*cos(2*acos(-goal.pose.orientation.w));
		...
		if (isCellInsideMap(startX, startY) && isCellInsideMap(goalX, goalY) && isCellInsideMap(turnX, turnY))
		{
			startCell = convertToCellIndex(startX, startY);

			turnCell = convertToCellIndex(turnX, turnY);
		...
		if (isStartAndGoalCellsValid(startCell, goalCell)){
			...
			vector<int> bestPath;
			bestPath.clear();
			bestPath = planner_func(startCell, goalCell, turnCell);
	...
	void dual_planner::getCorrdinate(float& x, float& y)
	{
	...
	int dual_planner::convertToCellIndex(float x, float y)
	{
	...
	void dual_planner::convertToCoordinate(int index, float& x, float& y)
	{
	...
	bool dual_planner::isCellInsideMap(float x, float y)
	{
		bool valid = true;
	...
	void dual_planner::mapToWorld(double mx, double my, double& wx, double& wy){
		costmap_2d::Costmap2D* costmap = costmap_ros_->getCostmap();
		wx = costmap->getOriginX() + mx * resolution;
		wy = costmap->getOriginY() + my * resolution;
	}

	vector<int> dual_planner::planner_func(int startCell, int goalCell, int turnCell){
		...
		bestPath.clear();
		tempPath.clear();
		bestPath=findPath(startCell, turnCell);
		bestPath.pop_back();
		tempPath=findPath(turnCell, goalCell);

		for(int i = 0; i < tempPath.size(); i++)
		{
			bestPath.push_back(tempPath[i]);
		}
	...
	/*********************************************************************************/
	vector<int> dual_planner::findPath(int startCell, int goalCell)
	{
	...
	vector <int> dual_planner::findFreeNeighborCell (int CellID){
	...
	/*********************************************************************************/
	bool dual_planner::isStartAndGoalCellsValid(int startCell,int goalCell)
	{ 
		bool isvalid=true;
	...
	bool  dual_planner::isFree(int i, int j){
	...
	//verify if the cell(i,j) is free
	bool  dual_planner::isFree(int CellID){
		return OGM[CellID];
	} 
}
;
